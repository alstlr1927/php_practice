<?php
    ...
    if (isset($_POST["mode"]) && $_POST["mode"] === "delete") {
        $num = $_POST["num"];
        $page = $_POST["page"];
        $sql = "select * from buy_board where num = $num";
        ...
        $sql = "delete from buy_board where num = $num";
	   ...
	   ";
        ...
        $sql = "insert into buy_board (id, name, subject, content, regist_day, hit,  file_name, file_type, file_copied) ";
	...
	";
        ...
        $sql = "update buy_board set subject='$subject', content='$content',  file_name='$upfile_name', file_type='$upfile_type', file_copied= '$copied_file_name'";
        $sql .= " where num=$num";
        mysqli_query($con, $sql);
	  ...
	  ";
        ...
        if (!$rowcount) {
            ...
            $sql = "INSERT INTO `buy_board_ripple` VALUES (null,'$q_parent','$q_userid','$q_username', '$q_usernick','$q_content','$regist_day')";
            $result = mysqli_query($con, $sql);
            if (!$result) {
                die('Error: ' . mysqli_error($con));
            }
        ...
        $sql = "DELETE FROM `buy_board_ripple` WHERE num=$q_num";
        $result = mysqli_query($con, $sql);
        if (!$result) {
            die('Error: ' . mysqli_error($con));
        }
